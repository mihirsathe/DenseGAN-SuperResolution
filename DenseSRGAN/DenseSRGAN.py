...
class DenseSRGAN:
    ...
    def show_size(self):
        print("W: {0} / H: {1} / C: {2}".format(self.imhr_w,
                                                self.imhr_h, self.imhr_c))
    ...
    def init_discriminator(self):
        ...
        hr_input = Input(shape=[self.imhr_w, self.imhr_h,
                                self.im_c], name='highres_input')
    ...
    def init_generator(self):
        ...
        lr_input = Input(shape=(self.imlr_w, self.imlr_h,
                                self.im_c), name='lowres_input')

        # Initial Convolution
        x = Conv2D(num_filts, (3, 3), padding='same',
                   name='init_conv', use_bias=False)(lr_input)
        ...
        # occurs in transistion block. Paper 3.1 says growth in Dense Block.

        # Dense Blocks
        # Consider 1st and last layer

        for blk_idx in range(1, self.num_dense_blks + 1):
            # Number of layers/block is constant blk_layers, can change per
            # block blk_layers[i] if param is list
            # num_filts += self.growth_rate

            # Grow maps
            x = db.dense_block(x, blk_idx, self.blk_layers,
        ...
        x = Conv2D(num_filts, (3, 3), padding='same',
                   name=base_name + '_c3x3s1', use_bias=False)(x)
        x = BatchNormalization(name=base_name + '_bn_1')(x)
        x = Activation('relu', name=base_name + '_relu_1')(x)
        x = Conv2D(num_filts, (1, 1), padding='same',
                   name=base_name + '_c1x1s1', use_bias=False)(x)
        x = BatchNormalization(name=base_name + '_bn_2')(x)
        x = Activation('relu', name=base_name + '_relu_2')(x)

        # x = BatchNormalization(epsilon=self.eps, name=base_name + '_bn')(x)
        # x = Activation('relu', name=base_name + '_relu')(x)
        x = Conv2D(self.im_c, (1, 1), name=base_name +
                   '_conv1', use_bias=False)(x)
    ...
    def build_models(self):
        ...
        # Create the model
        if self.gpu_list is not None:
            self.adv_model = multi_gpu_model(
                Model(im_lr, disc_gen_hr), gpus=self.gpu_list)
        else:
            self.adv_model = Model(im_lr, disc_gen_hr)
    ...
    def train(self, datahr=None, datalr=None,
        ...
        # This just throws away the remaining examples if batch_size not divisor of num_train
        num_train = len(self.datahr)
        num_batches = num_train / \
            batch_size if num_train % batch_size == 0 else num_train / batch_size - 1
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            for batch_idx in range(int(num_batches / 2)):  # Take 2 batches per round
                ...
                batch_lr = np.array([self.datalr[i, :, :, :]
                                     for i in idx[bix_begin:bix_end]])
                batch_hr = np.array([self.datahr[i, :, :, :]
                                     for i in idx[bix_begin:bix_end]])
                if verbose:
                ...
                log_mesg = "%d:%d: [D loss_hr: %f, acc_hr: %f, loss_gen: %f, acc_gen: %f]" % \
                    (epoch, batch_idx,
                     d_loss_hr[0], d_loss_hr[1], d_loss_gen[0], d_loss_gen[1])
                ...
                # print(log_mesg)
                running_loss.append(
                    [epoch] + [batch_idx] + list(d_loss_hr) + list(d_loss_gen) + list(a_loss))

            if epoch % view_interval == 0:
                print('Finished Epoch {0}... Time: {1}'.format(
                    epoch, datetime.datetime.now() - epoch_start_time))
                print(log_mesg)
            ...
            if epoch % save_interval == 0:
                ...
                plt.imshow(img)
                plt.savefig(
                    '{0}images/{1}bench_epoch_{2}'.format(self.dir_pfx, sub_dir, epoch))

            np.save(self.dir_pfx + 'loss_logging/' + sub_dir +
                    'loss_log.npy', arr=np.array(running_loss))

            if epoch % save_interval == 0:
                print('Saving weights at epoch {0}'.format(epoch))
                self.gen.save(self.dir_pfx + 'weights/' +
                              sub_dir + 'generator_weights.h5')
                self.disc.save(self.dir_pfx + 'weights/' +
                               sub_dir + 'discriminator_weights.h5')
