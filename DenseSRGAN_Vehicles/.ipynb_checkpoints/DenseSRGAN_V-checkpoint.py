...
class DenseSRGAN_V:
    ...
    def init_generator(self):
        ...
        x = Activation('tanh', name=base_name + '_tanh')(x)
    ...
    def train(self, datahr=None, datalr=None,
        ...
        for epoch in range(epochs):
            ...
            for batch_idx in range(int(num_batches/2)): # Take 2 batches per round
                ...
                y_tr = np.zeros((len(x_gen),) + (4,4,1)) + np.random.uniform(low=0.0, high=0.1, size=((len(x_gen),) + (4,4,1)))
                flipped_idx = np.random.choice(np.arange(len(y_tr)), size=int(noise_prop*len(y_tr)))
                y_tr[flipped_idx] = 1 - y_tr[flipped_idx]

                # Prepare labels for fake data
                y_gen = np.ones((len(x_gen),) + (4,4,1)) -  np.random.uniform(low=0.0, high=0.1, size=((len(x_gen),) + (4,4,1)))
                flipped_idx = np.random.choice(np.arange(len(y_gen)), size=int(noise_prop*len(y_gen)))
